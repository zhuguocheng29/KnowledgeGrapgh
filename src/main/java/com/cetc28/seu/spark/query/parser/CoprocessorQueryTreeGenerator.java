...
import com.cetc28.seu.spark.query.model.coprocessor.JoinerCoprocessorNode;
import com.cetc28.seu.spark.query.model.coprocessor.SimpleFilterCoprocessorNode;
import com.cetc28.seu.spark.query.model.coprocessor.SparqlFilterCoprocessorNode;
import com.cetc28.seu.sparql.FilterClause;
import com.cetc28.seu.sparql.SelectClause;

public class CoprocessorQueryTreeGenerator extends TreeGenerator{


	public CoprocessorQueryTreeGenerator(){
		
	}
	
    public CoprocessorQueryTreeGenerator(Configuration hbaseconf,JavaSparkContext sc){
	...
	public void generate(SelectClause sClause) {
		...
		if (joins == null) {
			...
			List<String> familyList =  ReturnQueryConditionAttributes(conditions.get(sub));
			qc.setFamilyList(familyList);
//			System.out.println(conditions.get(sub));
			SimpleFilterCoprocessorNode leafNode = new SimpleFilterCoprocessorNode(nodeid++, -1, -1, -1, sub, qc, sClause.getFilter());
			...
			if(rootRDF.getPredict().getValue().contains("objects"))
			{
				...
				JoinerCoprocessorNode rootNode = new JoinerCoprocessorNode();
			...
			else
			{
				...
				SparqlFilterCoprocessorNode leafNode=new SparqlFilterCoprocessorNode(nodeid++, -1, -1, -1, root,queryCondition, sClause.getFilter());
				nodeList.add(leafNode);
				tree.setRoot(leafNode);
			}
...
//			System.out.println(childRDF.getPredict().getValue() +" "+childRDF.getObject().getValue() + "===========");

			if(!childRDF.getPredict().getValue().contains("objects"))
			{
				//说明出现类似?s attributes:age ?v 可能
				QueryCondition queryCondition = new QueryCondition(childRDF);
				child = new SparqlFilterCoprocessorNode(nodeid++, -1, -1, parent.getId(), childTerm,queryCondition, filterClause);
				nodeList.add(child);
			}
			else
			{
				String[] predicate = childRDF.getPredict().getValue().split(":");
				child = new JoinerCoprocessorNode(nodeid++, parent.getId(), childTerm,childRDF,
						predicate[1]);
				nodeList.add(child);
			}
			...
			child = new SimpleFilterCoprocessorNode(nodeid++, -1, -1, parent.getId(), childTerm, qc, filterClause);
			nodeList.add(child);
			reduce(counts, childTerm);
			return child.getId();
		} else {
